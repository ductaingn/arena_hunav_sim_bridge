...
from arena_hunav_sim_bridge.hunav_sim_node_wrapper.stop_and_wait_timer_action import StopAndWaitTimerAction
...
from arena_hunav_sim_bridge.hunav_sim_node_wrapper.control_nodes import Sequence, Fallback
from arena_hunav_sim_bridge.ids_manager.goal_id_manager import GoalIDManager
from arena_hunav_sim_bridge.global_planner.path_finder import PathFinder, build_grid_from_world
...
class Planner:
    ...
    def __init__(
        ...
        self.planned_nodes = deepcopy(agent.nodes)  # Copy of original nodes to modify with waypoints
    ...
    def _find_parent_node(self, current_node: BTNode, node: BTNode) -> Sequence | Fallback | None:
        """
        Find the parent node of a given node in the agent's node tree.
        Returns a list of parent nodes in traversal order.
        """
        if self._is_control_node(current_node) and hasattr(current_node, 'children_nodes'):
    ...
    def _build_goto_graph(self) -> Tuple[Dict[int, List[int]], List[BTNode], Dict[int, BTNode]]:
        """
        ...
        goto_graph = {}
        goto_indices = [i for i, node in enumerate(significant_nodes) if isinstance(node, GoTo)]
    ...
    def plan(self):
        ...
        for i in range(1, len(goto_indices) - 1):
            ...
            if current_goal_pos and next_goal_pos:
                waypoints = self.path_finder.get_waypoints(current_goal_pos, next_goal_pos)
        ...
        if goto_indices:
            ...
            if first_goto_idx == 0:
                ...
                if first_goal_pos:
                    waypoints = self.path_finder.get_waypoints(start_pos, first_goal_pos)
                    agent_waypoints += waypoints
                    waypoints = waypoints[1:-1]  # Trim start and end
    ...
    def _insert_waypoints_before_goto(self, goto_node: GoTo, waypoints: List[Tuple[float, float]]):
        """
        ...
        if parent_node is None:
            raise ValueError("Target GoTo node not found in planned nodes for insertion.")


class MultiAgentPlanner:
    def __init__(
        self, agents: List[Agent], goal_id_manager: GoalIDManager, world: World
    ):
        self.agents = agents
        self.goal_id_manager = goal_id_manager
